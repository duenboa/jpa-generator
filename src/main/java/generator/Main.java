...
//        basePath = new File("").getCanonicalPath();
    ...
    public Main(String basePath, List<GenTable> genTableList) {
    ...
    private List<GenTable> initGenTableList() {
        ...
        if (tableNameList != null && tableNameList.size() > 0) {
            for (String tableName : tableNameList) {
                ...
                genTable.setDaoPackage(ConfigPropertiesUtil.getProperty("daoPackage"));
                ...
                genTable.setDaoPath(getDaoPath(basePath, genTable.getDaoPackage(), genTable.getClassName()));
     ...
     * @return
     */
    public String getDaoPath(String basePath, String daoPackage, String className) {
        return basePath + "\\" + daoPackage.replace(".", "\\") + "\\" + className + "Dao.java";
    }
     ...
     * @param genTable
     */
    public void generateDaoFile(GenTable genTable) {
        try {
            //创建model文件
            Generator generator = new DaoGenerator(genTable); // model生成器
            generator.generateFile();
        } catch (Exception e) {
            new RuntimeException("创建" + genTable.getDaoPath() + "文件时出错！");
            e.printStackTrace();
        } finally {
            System.out.println(genTable.getDaoPath() + "文件创建完毕！");
        }
    }
